 ...
 // const { isLoggedIn } = useAuth();
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const categoriesResponse = await axios.get(`http://167.71.84.130:3002/get-categories/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  const handleAddExpense = async () => {
    try {
      ...
      await axios.post('http://167.71.84.130:3002/add-expense', {
        userId: userId,
        month: selectedMonth,
  ...
  return (
    <main className="expense-manager-container" id="main" aria-label="main">
    <div className="expense-form">
      ...
      <input
        ...
        style={{marginRight:'15px'}}
      />
         {expenseAdded && <p className="message">Successfully added expense!</p>}
