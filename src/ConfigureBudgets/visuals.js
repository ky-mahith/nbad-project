...
function Visuals() {
  ...
  useEffect(() => {
    ...
    if (selectedMonth) {
      axios
        .get(`http://142.93.116.20:3002/get-budgets/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  useEffect(()=>{
    ...
    if (selectedMonth) {
      axios
        .get(`http://142.93.116.20:3002/get-expenses/${userId}?month=${selectedMonth}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
